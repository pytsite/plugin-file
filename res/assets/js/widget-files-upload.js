define(['jquery', 'assetman', 'http-api', 'lang', 'load-image', 'jquery-ui'], function ($, assetman, httpApi, lang) {
    return function (widget) {
        ...
        assetman.loadCSS('plugins.file@css/widget-files-upload.css');
        ...
        function removeSlot(slot, confirmDelete) {
            ...
            if (confirmDelete !== false && !confirm(lang.t('file@really_delete')))
                return;
        ...
        function uploadFile(file) {
            if (acceptedFileTypes !== '*/*' && file.type.split('/')[0] !== acceptedFileTypes) {
                alert(lang.t('file@file_has_invalid_type'));
                return false;
            }

            if (maxFileSize && file.size > maxFileSize) {
                alert(lang.t('file@file_too_big', {
                    file_name: file.name,
            ...
            if (filesCount > maxFiles) {
                filesCount = maxFiles;
                progressSlot.hide();
                alert(lang.t('file@max_files_exceeded'));
